      ...
      {tab === 'logs' && <div style={{ padding: '14px 16px' }}>
        ...
        {stampLogs.length === 0 ? <C style={{ textAlign: 'center', padding: 30 }}><div style={{ color: COLORS.gray }}>Henüz işlem yok</div></C> : stampLogs.map((l, i) => <div key={l.id || i} style={{ background: COLORS.cardBg, borderRadius: 14, padding: '12px 16px', marginBottom: 6, border: `1px solid ${COLORS.divider}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <B text={l.type === 'stamp' ? 'DAMGA' : l.type === 'goat_monthly' ? 'GOAT' : l.type === 'admin_add' ? '+ADMIN' : l.type === 'admin_remove' ? '-ADMIN' : l.type === 'referral_bonus' ? 'REFERANS' : 'ÜCRETSİZ'} color={l.type === 'stamp' ? COLORS.fioreOrange : l.type === 'goat_monthly' ? COLORS.gold : l.type?.startsWith('admin') ? COLORS.purple : l.type === 'referral_bonus' ? COLORS.blue : COLORS.green} />
          </div>
